...
class FunctionContext:
  ...
  def _register_ai_function(self, fnc: Callable) -> None:
    if not hasattr(fnc, METADATA_ATTR):
      logging.warning(f"function {fnc.__name__} does not have ai metadata")
      return
    ...
    for name, param in sig.parameters.items():
      ...
      if param.annotation is inspect.Parameter.empty:
        raise ValueError(f"{fnc_name}: missing type annotation for parameter {name}")

      th = type_hints[name]
      if not is_type_supported(th):
        raise ValueError(f"{fnc_name}: unsupported type {th} for parameter {name}")
